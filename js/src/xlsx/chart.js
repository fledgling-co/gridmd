...
import { resolveColor } from './units.js';
...
export function chartSpaceXml(chart, { ownSheet, tableIndex, themeColors, report }) {
  const meta = chart.meta ?? {};
  const type = chart.type ?? 'column';
  const ref = (r) => resolveDataRef(r, ownSheet, tableIndex);
  ...
  const solidFill = (rgb) => rgb ? `<a:solidFill><a:srgbClr val="${rgb.slice(2)}"/></a:solidFill>` : '';
  const spPrFor = (s, isLine) => {
  ...
  const dLblsXml = (l) => {
    ...
    const POS = { center: 'ctr', 'inside-end': 'inEnd', 'inside-base': 'inBase', 'outside-end': 'outEnd', above: 't' };
  ...
  const trendlineXml = (t) => {
    if (!t) return '';
    const TYPES = { linear: 'linear', poly: 'poly', exp: 'exp', log: 'log', power: 'power', 'moving-average': 'movingAvg' };
    return '<c:trendline>'
  ...
  const errBarsXml = (e) => {
    if (!e) return '';
    const TYPES = { 'std-error': 'stdErr', percentage: 'percentage', 'std-dev': 'stdDev', fixed: 'fixedVal' };
  ...
  const catXml = (s) => {
    const c = s.cat ?? series.find((x) => x.cat)?.cat;
    if (!c) return '';
    const f = ref(c);
    return f ? `<c:cat><c:strRef><c:f>${esc(f)}</c:f></c:strRef></c:cat>` : '';
  };
  const valXml = (s, tag = 'c:val') => {
    const f = ref(s.val);
  ...
  const catAxXml = (axId, crossId, del, m = {}) =>
  ...
  const valAxXml = (axId, crossId, pos, m = {}, crossesMax = false) =>
  ...
  const LEGEND_POS = { left: 'l', right: 'r', top: 't', bottom: 'b' };
...
function XMLD() { return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'; }
