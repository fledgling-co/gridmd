...
import { parseTarget } from '../refs.js';
...
const SHAPE_PRESETS = new Map(Object.entries({
...
export function parseAnchor(anchorStr, size) {
...
function wrapAnchor(anchor, inner) {
  if (anchor.kind === 'two') {
...
export function chartFrame(id, name, relId, anchor) {
...
export function pictureFrame(id, name, relId, anchor, alt) {
...
export function shapeFrame(id, shape, anchor, themeColors) {
  ...
  const lines = String(meta.text ?? '').replace(/\n$/, '').split('\n');
