...
import { parseCell, numToCol } from '../refs';
import { one, all, textOf, attr } from '../xml';
import type { XmlNode } from '../types';

const A1 = (col: number, row: number): string => `${numToCol(col)}${row}`;

export interface AnchorSpec { at: string; size?: { w: number; h: number } }
export interface DrawingAnchor { kind: string; el: XmlNode }
export type MediaLookup = (rid: string) => { mime: string; data: Buffer } | null;

const LEGEND_REV = new Map<string, string>(Object.entries({ l: 'left', r: 'right', t: 'top', b: 'bottom' }));
const DLBL_POS_REV = new Map<string, string>(Object.entries({ ctr: 'center', inEnd: 'inside-end', inBase: 'inside-base', outEnd: 'outside-end', t: 'above' }));
const TREND_REV = new Map<string, string>(Object.entries({ linear: 'linear', poly: 'poly', exp: 'exp', log: 'log', power: 'power', movingAvg: 'moving-average' }));
const ERR_TYPE_REV = new Map<string, string>(Object.entries({ stdErr: 'std-error', percentage: 'percentage', stdDev: 'std-dev', fixedVal: 'fixed' }));
const SHOW_AS_REV = new Map<string, string>(Object.entries({
...
const AGG_REV = new Map<string, string>(Object.entries({
...
const PRESET_REV = new Map<string, string>(Object.entries({
...
// ---- drawing anchors ----
export function anchorsOf(drawingDoc: XmlNode | null): DrawingAnchor[] {
...
export function anchorText(anchor: DrawingAnchor): AnchorSpec {
...
const fenceAnchor = (a: AnchorSpec): string => a.at === 'sheet' ? 'at sheet' : `at ${a.at}${a.size ? ` size ${a.size.w}x${a.size.h}` : ''}`;

// ---- chart reversal ----
const BLOCK_KINDS = new Map<string, string>(Object.entries({
...
const CX_TYPE_REV = new Map<string, string>(Object.entries({
  treemap: 'treemap', sunburst: 'sunburst', waterfall: 'waterfall',
  funnel: 'funnel', boxWhisker: 'box-whisker', regionMap: 'map',
}));

interface ChartBlock { el: XmlNode; kind: string; threeD: boolean }
interface ChartSeries { ser: XmlNode; kind: string; secondary: boolean; gap: string | undefined; overlap: string | undefined }

export function reverseChart(chartDoc: XmlNode, anchorSpec: AnchorSpec): string[] | null {
  ...
  const secondAxIds = valAxes.length > 1 ? new Set([attrVal(valAxes[1]!, 'axId')]) : new Set<string | undefined>();
  ...
  for (const { ser, kind, secondary, gap, overlap } of series) {
    ...
    const refOf = (el: XmlNode | null): string | undefined => el ? textOf(one(one(el, 'strRef') ?? one(el, 'numRef') ?? el, 'f')).trim() : undefined;
  ...
  const axisYaml = (ax: XmlNode | null, isCat: boolean): string | null => {
    if (!ax) return null;
    const bits: string[] = [];
    ...
    if (one(scaling, 'orientation')?.attrs.val === 'maxMin') bits.push('reverse: true');
  ...
  const y = axisYaml(valAxes[0] ?? null, false);
  const y2 = axisYaml(valAxes[1] ?? null, false);
...
function attrVal(el: XmlNode, childName: string): string | undefined {
  const c = one(el, childName);
  return c?.attrs.val;
}

function lineColor(spPr: XmlNode | null): string | null {
  const ln = spPr && one(spPr, 'ln');
  return ln ? solidFillColor(ln) : null;
}

// ---- picture reversal ----
export function reversePicture(pic: XmlNode, anchorSpec: AnchorSpec, mediaLookup: MediaLookup): string[] | null {
  const blip = one(one(pic, 'blipFill'), 'blip');
  const relId = blip && attr(blip, 'embed');
  const media = relId && mediaLookup(relId);
  if (!media) return null;
  const alt = one(one(pic, 'nvPicPr'), 'cNvPr')?.attrs.descr;
...
export function reverseShape(sp: XmlNode, anchorSpec: AnchorSpec): string[] {
  const spPr = one(sp, 'spPr');
  const preset = one(spPr, 'prstGeom')?.attrs.prst;
  ...
  const firstRPr = txBody && one(all(one(txBody, 'p'), 'r')[0], 'rPr');
  if (firstRPr) {
    const bits: string[] = [];
...
// ---- pivot reversal ----
export function reversePivot(ptDoc: XmlNode, cacheDoc: XmlNode | null): string[] | null {
  const loc = one(ptDoc, 'location');
  if (!loc || !cacheDoc) return null;
  const fields = all(one(cacheDoc, 'cacheFields'), 'cacheField').map((f) => f.attrs.name);
  if (!fields.length) return null;
  const anchor = (loc.attrs.ref ?? '').split(':')[0]!;
  const src = one(one(cacheDoc, 'cacheSource'), 'worksheetSource');
  ...
  const fieldList = (elName: string): string[] => all(one(ptDoc, elName), 'field')
    .map((f) => fields[Number(f.attrs.x)]).filter((f): f is string => Boolean(f));
  ...
  const pages = all(one(ptDoc, 'pageFields'), 'pageField')
    .map((f) => fields[Number(f.attrs.fld)]).filter((f): f is string => Boolean(f));
  if (pages.length) lines.push('filters:', ...pages.map((f) => `  - { field: ${yaml(f)} }`));
  const dataFields = all(one(ptDoc, 'dataFields'), 'dataField');
...
// ---- slicer reversal ----
export function reverseSlicer(slicerEl: XmlNode, cacheDoc: XmlNode | null, tableNameById: Map<number, string | undefined>, anchorSpec: AnchorSpec | undefined): string[] | null {
...
function findDeep(el: XmlNode | null, name: string): XmlNode | null {
  if (!el) return null;
...
export function reverseChartEx(doc: XmlNode, anchorSpec: AnchorSpec): string[] | null {
  const chart = one(doc, 'chart');
  const region = chart && one(one(chart, 'plotArea'), 'plotAreaRegion');
  ...
  if (!type) return null;
  const data = one(one(doc, 'chartData'), 'data');
  ...
  if (legend) {
    const POS: Record<string, string> = { l: 'left', r: 'right', t: 'top', b: 'bottom' };
