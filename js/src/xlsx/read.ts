...
import { parseXml, attr, one, all, textOf, findDeep } from '../xml';
import { numToCol, colToNum, parseCell } from '../refs';
import { DEFAULT_THEME } from './units';
import { anchorsOf, anchorText, reverseChart, reversePicture, reverseShape, reversePivot, reverseSlicer } from './read-objects';
import type { AnchorSpec } from './read-objects';
import type { CellPos, Meta, ReportEntry, XmlNode } from '../types';

const DAY_MS = 86400000;

const BUILTIN_NUMFMTS = new Map<number, string>([
...
const PAPER_REV = new Map<number, string>([[1, 'letter'], [3, 'tabloid'], [5, 'legal'], [8, 'A3'], [9, 'A4'], [11, 'A5']]);
const CELLIS_REV = new Map<string, string>(Object.entries({
  equal: '=', notEqual: '<>', greaterThan: '>', greaterThanOrEqual: '>=',
  lessThan: '<', lessThanOrEqual: '<=',
}));
const TIME_PERIOD_REV = new Map<string, string>(Object.entries({
...
const ICON_REV = new Map<string, string>([
...
const SUBTOTAL_REV = new Map<string, number>([['average', 101], ['countNums', 102], ['count', 103],
  ['max', 104], ['min', 105], ['product', 106], ['stdDev', 107], ['sum', 109], ['var', 110]]);
const PROTECT_REV = new Map<string, string>(Object.entries({
...
const BORDER_STYLE_REV = new Map<string, string>(Object.entries({
...
interface RichRun { text: string; bold?: boolean; italic?: boolean; size?: number; color?: string }
...
interface RelInfo { target: string; type: string | undefined; mode: string | undefined }
interface DefinedName { name: string | undefined; value: string; localId: number | null; hidden: boolean }
interface PageName { area?: string; titles?: string }
interface TableRange { c1: number; r1: number; c2: number; r2: number }
interface SharedItem { text?: string; rich?: RichRun[] }
interface ThreadItem {
  ref: string | undefined; by: string; at: string | undefined; text: string;
  done: boolean; id: string | undefined; parentId: string | undefined; replies?: ThreadItem[];
}
interface StyleReader { propsForXf(idx: number): Meta; dxfProps(idx: number): Meta }

export interface ImportResult { gmd: string; report: ReportEntry[] }

export function xlsxToGridmd(buffer: Buffer): ImportResult {
  const parts = zipRead(buffer);
  const report: ReportEntry[] = [];
  const consumed = new Set<string>();
  const xml = (name: string): XmlNode | null => {
    if (!parts.has(name)) return null;
    consumed.add(name);
    return parseXml(parts.get(name)!.toString('utf8'));
  };
  const relsFor = (partName: string): Map<string, RelInfo> => {
  ...
  const tableNameById = new Map<number, string | undefined>();
  ...
  const slicerCaches = new Map<string, { part: string; doc: XmlNode }>(); // cache name -> { part, doc }
  for (const name of parts.keys()) {
  ...
  const timelineCaches = new Map<string, { part: string; doc: XmlNode }>(); // cache name -> { part, doc }
  for (const name of parts.keys()) {
  ...
  const pageNames = new Map<number, PageName>(); // sheetIdx -> { printArea, printTitles }
  for (const dn of all(one(wb, 'definedNames'), 'definedName')) {
  ...
  sheetsEl.forEach((sh, sheetIdx) => {
    ...
    if (ws) body.push(...renderSheet(sh, ws, wsRels, sheetIdx));
  });
  ...
  // ================= per-sheet =================
  function renderSheet(sh: XmlNode, ws: XmlNode, wsRels: Map<string, RelInfo>, sheetIdx: number): string[] {
    const out = [`# ${sh.attrs.name}`, ''];
    const cellsByRef = new Map<string, ImportCell>();
    const arrayAnchors: ImportCell[] = [];
    ...
    for (const r of all(one(ws, 'sheetData'), 'row')) {
      const bits: string[] = [];
    ...
    const slicerAnchors = new Map<string, AnchorSpec>();
    for (const [, rel] of wsRels) {
      if (!/^xl\/drawings\/drawing\d+\.xml$/.test(rel.target)) continue;
      const dDoc = xml(rel.target);
      if (!dDoc) continue;
      const dRels = relsFor(rel.target);
      const mediaLookup = (rid: string): { mime: string; data: Buffer } | null => {
    ...
    for (const [, rel] of wsRels) {
      ...
      for (const sl of all(sDoc, 'slicer')) {
        const cache = sl.attrs.cache ? slicerCaches.get(sl.attrs.cache) : undefined;
        const lines = cache && reverseSlicer(sl, cache.doc, tableNameById, sl.attrs.name ? slicerAnchors.get(sl.attrs.name) : undefined);
    ...
    for (const [, rel] of wsRels) {
      ...
      for (const tl of all(tDoc, 'timeline')) {
        const cache = tl.attrs.cache ? timelineCaches.get(tl.attrs.cache) : undefined;
        const pivotName = cache && one(one(cache.doc, 'pivotTables'), 'pivotTable')?.attrs.name;
        if (!pivotName) {
          report.push({ feature: rel.target, action: 'carried', note: 'unrecognized timeline form — carried as {raw}' });
          continue;
        }
        consumed.add(cache!.part);
        const spec = (tl.attrs.name ? slicerAnchors.get(tl.attrs.name) : undefined) ?? { at: 'A1', size: { w: 320, h: 110 } };
    ...
    const spillCovered = new Set<string>();
    ...
    const inTable = (pos: CellPos): boolean => tableRanges.some((tr) => pos.col >= tr.c1 && pos.col <= tr.c2 && pos.row >= tr.r1 && pos.row <= tr.r2);

    // hyperlinks / notes / threads attach to cells
    const linkByRef = new Map<string, { link: string; tip: string | undefined }>();
    for (const h of all(one(ws, 'hyperlinks'), 'hyperlink')) {
      const target = h.attrs.location ? `#${h.attrs.location}` : wsRels.get(attr(h, 'id') ?? '')?.target;
      if (target && h.attrs.ref) linkByRef.set(h.attrs.ref, { link: target, tip: h.attrs.tooltip });
    }
    const noteByRef = new Map<string, string>();
    const threads: ThreadItem[] = [];
    const threadRefs = new Set<string | undefined>();
    for (const [, rel] of wsRels) {
      if (rel.target.startsWith('xl/threadedComments/')) {
        const tc = xml(rel.target);
        const personsMap = readPersons();
        const roots = new Map<string | undefined, ThreadItem>();
    ...
    for (const [, rel] of wsRels) {
      if (/^xl\/comments\d+\.xml$/.test(rel.target)) {
        ...
        for (const el of all(one(cm, 'commentList'), 'comment')) {
          ...
          if (el.attrs.ref) noteByRef.set(el.attrs.ref, textOf(one(el, 'text')));
        }
      }
    ...
    for (const { ref, cell, pos } of ordered) {
      ...
      if (note !== undefined) notesEmitted.add(ref);
    ...
    // Notes on cells with no content (annotation-only, SPEC §9.4) — a
    // comments-part ref need not have a sheetData cell.
    for (const [ref, note] of noteByRef) {
      if (notesEmitted.has(ref) || !parseCell(ref)) continue;
      out.push(`@ ${ref}`, '  note: |');
      for (const l of String(note).split('\n')) out.push(`    ${l}`);
    }
    if (ordered.length || noteByRef.size > notesEmitted.size) out.push('');

    // merges
    for (const m of all(one(ws, 'mergeCells'), 'mergeCell')) {
    ...
    for (const cf of all(ws, 'conditionalFormatting')) {
      const rules = all(cf, 'cfRule').map(cfRuleToYaml).filter((r): r is string[] => r !== null);
      if (rules.length) out.push(`\`\`\`{cf} ${cf.attrs.sqref}`, ...rules.flat(), '```', '');
    }

    // validations
    const dv = one(ws, 'dataValidations');
    for (const v of all(dv, 'dataValidation')) {
    ...
    // sparklines
    for (const ext of all(one(ws, 'extLst'), 'ext')) {
    ...
    // scenarios
    for (const sc of all(one(ws, 'scenarios'), 'scenario')) {
  ...
  // ================= cell machinery =================
  function readCell(c: XmlNode, pos: CellPos): ImportCell {
    void pos;
    const t = c.attrs.t ?? 'n';
    ...
    const vEl = one(c, 'v');
    const isEl = one(c, 'is');
    const cell: ImportCell = { props, formula: null, arrayRef: null, cachedText: null, scalarText: null, rich: null };
    ...
    if (isEl) {
      ...
      if (runs.length > 1 || (runs.length === 1 && one(runs[0], 'rPr'))) {
        cell.rich = runs.map((run) => {
          ...
          const item: RichRun = { text: textOf(one(run, 't')) };
          if (pr) {
  ...
  function sharedText(idx: number, cell: ImportCell): string | null {
    const si = shared[idx];
    if (!si) return '""';
  ...
  function scalarTextOf(cell: ImportCell): string {
    return cell.scalarText ?? cell.cachedText ?? '';
  }

  function cellLine(ref: string, cell: ImportCell, extras: { link?: string; tip?: string }, note: string | undefined): string[] | null {
  ...
  // ================= directive reversal =================
  function renderTable(t: XmlNode, cellsByRef: Map<string, ImportCell>, tableRanges: TableRange[]): string[] {
    ...
    columns.forEach((name, i) => {
      const fmts = new Set<string | undefined>();
  ...
  function cfRuleToYaml(rule: XmlNode): string[] | null {
    ...
    const operand = (f: string): string => /^-?\d+(\.\d+)?$/.test(f) ? f : /^".*"$/.test(f) ? f.slice(1, -1).replace(/""/g, '"') : `=${f}`;
  ...
  function validationToBlock(v: XmlNode): string[] {
    const TYPE_REV: Record<string, string> = { list: 'list', whole: 'whole', decimal: 'decimal', date: 'date', time: 'time', textLength: 'text-length', custom: 'custom' };
    const out = [`\`\`\`{validation} ${v.attrs.sqref}`, `type: ${TYPE_REV[v.attrs.type ?? ''] ?? 'custom'}`];
    const f1 = textOf(one(v, 'formula1'));
    const f2 = textOf(one(v, 'formula2'));
    if (v.attrs.type === 'list') {
      ...
      const OP_REV: Record<string, string> = { equal: '=', notEqual: '<>', greaterThan: '>', greaterThanOrEqual: '>=', lessThan: '<', lessThanOrEqual: '<=' };
  ...
  function filterToBlock(af: XmlNode, sortState: XmlNode | null): string[] {
    ...
    for (const fc of all(af, 'filterColumn')) {
      ...
      if (filters) cols.push(`  ${letter}: { values: [${all(filters, 'filter').map((f) => yamlStr(f.attrs.val)).join(', ')}] }`);
        ...
        const OP_REV: Record<string, string> = { equal: '=', notEqual: '<>', greaterThan: '>', greaterThanOrEqual: '>=', lessThan: '<', lessThanOrEqual: '<=' };
        if (cf && cf.attrs.operator && OP_REV[cf.attrs.operator]) cols.push(`  ${letter}: { op: "${OP_REV[cf.attrs.operator]}", value: ${yamlMaybeNum(cf.attrs.val)} }`);
      }
    }
  ...
  function sparklineToBlock(g: XmlNode): string[] {
  ...
  function pageToBlock(ws: XmlNode, pn: PageName | undefined): string[] {
    ...
    if (margins) {
      const cm = (v: string | undefined): number => Math.round(Number(v) * 2.54 * 100) / 100;
    ...
    const hfParse = (s: string): { left: string; center: string; right: string } => {
      const seg: { left: string; center: string; right: string } = { left: '', center: '', right: '' };
    ...
    for (const [el, key] of [[one(hfEl, 'oddHeader'), 'header'], [one(hfEl, 'oddFooter'), 'footer']] as const) {
      if (!el) continue;
  ...
  function readPersons(): Map<string, string | undefined> {
    const map = new Map<string, string | undefined>();
    const doc = xml('xl/persons/person.xml');
    if (doc) for (const p of all(doc, 'person')) if (p.attrs.id) map.set(p.attrs.id, p.attrs.displayName);
    return map;
  }
}

// ================= styles =================
function readStyles(doc: XmlNode | null, themeColors: Record<string, string>): StyleReader {
  const numFmts = new Map<number, string>(BUILTIN_NUMFMTS);
  const fonts: Meta[] = [];
  const fills: Meta[] = [];
  const borders: Meta[] = [];
  const xfs: XmlNode[] = [];
  const dxfs: XmlNode[] = [];
  if (doc) {
    for (const nf of all(one(doc, 'numFmts'), 'numFmt')) {
      numFmts.set(Number(nf.attrs.numFmtId), nf.attrs.formatCode!);
    }
    for (const f of all(one(doc, 'fonts'), 'font')) fonts.push(fontProps(f, themeColors));
    for (const f of all(one(doc, 'fills'), 'fill')) fills.push(fillProps(f, themeColors));
    for (const b of all(one(doc, 'borders'), 'border')) borders.push(borderProps(b, themeColors));
    for (const x of all(one(doc, 'cellXfs'), 'xf')) xfs.push(x);
    for (const d of all(one(doc, 'dxfs'), 'dxf')) dxfs.push(d);
  }
  return {
    propsForXf(idx: number): Meta {
      const x = xfs[idx];
      if (!x || idx === 0) return {};
      const props: Meta = {};
      ...
      if (al) {
        const H: Record<string, string> = { left: 'left', center: 'center', right: 'right', justify: 'justify', fill: 'fill', centerContinuous: 'center-across', distributed: 'distributed' };
        const V: Record<string, string> = { top: 'top', center: 'middle', bottom: 'bottom', justify: 'justify', distributed: 'distributed' };
...
function fontProps(f: XmlNode, themeColors: Record<string, string>, isDxf = false): Meta {
  void isDxf;
  const props: Meta = {};
...
function fillProps(f: XmlNode, themeColors: Record<string, string>): Meta {
...
function borderProps(b: XmlNode, themeColors: Record<string, string>): Meta {
  const props: Meta = {};
  for (const [el, key] of [['top', 'border-top'], ['bottom', 'border-bottom'], ['left', 'border-left'], ['right', 'border-right']] as const) {
    const e = one(b, el);
    if (e?.attrs.style) {
...
function readSharedStrings(doc: XmlNode | null): SharedItem[] {
  if (!doc) return [];
  return all(doc, 'si').map((si): SharedItem => {
    const runs = all(si, 'r');
    if (runs.length > 1 || (runs.length === 1 && one(runs[0], 'rPr'))) {
      return {
        rich: runs.map((run): RichRun => {
          const pr = one(run, 'rPr');
          const item: RichRun = { text: textOf(one(run, 't')) };
          if (pr) {
            if (one(pr, 'b')) item.bold = true;
...
function readTheme(doc: XmlNode | null): Record<string, string> {
...
function colorFrom(el: XmlNode | null, themeColors: Record<string, string>): string | null {
  if (!el) return null;
  if (el.attrs.auto === '1') return null;
  if (el.attrs.rgb) {
    const argb = el.attrs.rgb.toUpperCase();
    return argb.length === 8
      ? (argb.startsWith('FF') ? `#${argb.slice(2)}` : `#${argb.slice(2)}${argb.slice(0, 2)}`)
      : `#${argb}`;
  }
  if (el.attrs.theme === undefined) return null;
  const slot = THEME_SLOTS[Number(el.attrs.theme)];
  const tint = el.attrs.tint !== undefined ? Math.round(Number(el.attrs.tint) * 100) : 0;
  return !slot ? null : tint ? `${slot}@${tint}` : slot;
}
...
function quoteText(s: string, forceQuote: boolean): string {
  if (s === '') return '""';
...
const yamlMaybeNum = (s: unknown): string => NUMBER_RE.test(String(s)) ? String(s) : yamlStr(s);

function yamlPropVal(v: unknown): string {
  if (typeof v === 'boolean' || typeof v === 'number') return String(v);
  return yamlStr(v);
}

function bodyValue(scalarText: string): string {
...
function classifyName(value: string): string {
...
function serialToIso(serial: number, dateSystem: number, code: string): string | null {
  void code;
