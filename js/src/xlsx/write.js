...
const ICON_SETS = new Map(Object.entries({
...
const SUBTOTAL_FN = new Map([[101, 'average'], [102, 'countNums'], [103, 'count'],
  [104, 'max'], [105, 'min'], [106, 'product'], [107, 'stdDev'], [109, 'sum'], [110, 'var']]);
const CELLIS_OPS = new Map(Object.entries({
  '=': 'equal', '<>': 'notEqual', '>': 'greaterThan', '>=': 'greaterThanOrEqual',
  '<': 'lessThan', '<=': 'lessThanOrEqual',
}));
const FILTER_OPS = new Map(Object.entries({
  '=': 'equal', '<>': 'notEqual', '>': 'greaterThan', '>=': 'greaterThanOrEqual',
  '<': 'lessThan', '<=': 'lessThanOrEqual',
}));
const TIME_PERIODS = new Map(Object.entries({
...
const PROTECT_ATTRS = new Map(Object.entries({
...
export function writeXlsx(model) {
  ...
  const addPart = (name, data, contentType) => {
  ...
  addPart('xl/_rels/workbook.xml.rels', relsXml(wbRels));
  ...
  const contentTypes = XMLDECL
    ...
    + [...mediaDefaults].map((s) => JSON.parse(s)).map(([ext, ct]) => `<Default Extension="${ext}" ContentType="${ct}"/>`).join('')
  ...
  function renderSheet(s) {
    ...
    const rp = (r) => { if (!rowProps.has(r)) rowProps.set(r, {}); return rowProps.get(r); };
    for (const [k, v] of Object.entries(s.meta.rows ?? {})) {
      ...
      for (let r = a; r <= (b ?? a); r++) {
        ...
        if (typeof v === 'number') o.ht = pxToPt(v);
          ...
          if (v.height !== undefined) o.ht = pxToPt(v.height);
          if (v.hidden) o.hidden = true;
          if (v.group) o.level = v.group;
        }
      }
    }
    ...
    for (const [k, v] of Object.entries(s.meta.cols ?? {})) {
      ...
      const cfg = typeof v === 'number' ? { width: v } : (v ?? {});
    ...
    if (s.filters.length) {
      ...
      afXml = `<autoFilter ref="${esc(f.sqref)}">${Object.entries(f.meta.cols ?? {}).map(([letter, crit]) => {
        ...
        if (crit.values) return `<filterColumn colId="${colId}"><filters>${crit.values.map((v) => `<filter val="${esc(v)}"/>`).join('')}</filters></filterColumn>`;
        if (crit.top !== undefined) {
          const raw = String(crit.top ?? crit.bottom);
          return `<filterColumn colId="${colId}"><top10 val="${parseInt(raw, 10)}"${raw.endsWith('%') ? ' percent="1"' : ''}/></filterColumn>`;
        }
        if (crit.op !== undefined && FILTER_OPS.has(crit.op)) {
          return `<filterColumn colId="${colId}"><customFilters><customFilter operator="${FILTER_OPS.get(crit.op)}" val="${esc(crit.value)}"/></customFilters></filterColumn>`;
        }
      ...
      if (Array.isArray(f.meta.sort) && f.meta.sort.length) {
        ...
        const conds = f.meta.sort.filter((lvl) => (lvl.by ?? 'value') === 'value').map((lvl) => {
    ...
    const cfXml = s.cf.map((block) => {
      const rules = (Array.isArray(block.rules) ? block.rules : []).map((rule) =>
    ...
    if (s.meta.protect?.enabled) {
      const allow = new Set(s.meta.protect.allow ?? []);
      const attrs = ['sheet="1"'];
    ...
    const breaksXml = (pg.breaks?.rows?.length ? `<rowBreaks count="${pg.breaks.rows.length}" manualBreakCount="${pg.breaks.rows.length}">${pg.breaks.rows.map((r) => `<brk id="${r}" max="16383" man="1"/>`).join('')}</rowBreaks>` : '')
      + (pg.breaks?.cols?.length ? `<colBreaks count="${pg.breaks.cols.length}" manualBreakCount="${pg.breaks.cols.length}">${pg.breaks.cols.map((c) => `<brk id="${c}" max="1048575" man="1"/>`).join('')}</colBreaks>` : '');
    ...
    for (const p of s.pivots) {
      ...
      addPart(`xl/pivotCache/_rels/pivotCacheDefinition${pivotCount}.xml.rels`,
        relsXml([`<Relationship Id="rId1" Type="${REL.pivotCacheRec}" Target="pivotCacheRecords${pivotCount}.xml"/>`]));
      ...
      addPart(`xl/pivotTables/_rels/pivotTable${pivotCount}.xml.rels`,
        relsXml([`<Relationship Id="rId1" Type="${REL.pivotCacheDef}" Target="../pivotCache/pivotCacheDefinition${pivotCount}.xml"/>`]));
      const rid = nextRel();
    ...
    if (s.tables.length) {
      ...
      for (const t of s.tables) {
        ...
        const filterCols = Object.entries(t.filter ?? {}).map(([colName, crit]) => {
          const ci = t.columns.findIndex((c) => c.toLowerCase() === colName.toLowerCase());
          if (ci === -1 || !crit?.values) return '';
          return `<filterColumn colId="${ci}"><filters>${crit.values.map((v) => `<filter val="${esc(v)}"/>`).join('')}</filters></filterColumn>`;
        }).join('');
    ...
    if (anchors.length) {
      ...
      if (drawingRels.length) addPart(`xl/drawings/_rels/drawing${drawingCount}.xml.rels`, relsXml(drawingRels));
      const rid = nextRel();
    ...
    if (rels.length) addPart(`xl/worksheets/_rels/sheet${wsCount}.xml.rels`, relsXml(rels));
    return { target: `worksheets/sheet${wsCount}.xml` };
  }

  // ================= chart sheet =================
  function renderChartSheet(s) {
    csCount++;
    ...
    if (rels.length) addPart(`xl/chartsheets/_rels/sheet${csCount}.xml.rels`, relsXml(rels));
  ...
  function sparklineGroupXml(g, ownSheet) {
    const meta = g.meta ?? {};
  ...
  function cellXml(cell, content, xf) {
    ...
    if (content.formula !== undefined) {
      ...
      if (cached.kind === 'date' || cached.kind === 'time') return `<c r="${ref}"${sAttr}>${f}<v>${isoToSerial(cached.value, dateSystem)}</v></c>`;
    ...
    switch (sc.kind) {
      ...
      case 'date':
      case 'time': return `<c r="${ref}"${sAttr}><v>${isoToSerial(sc.value, dateSystem)}</v></c>`;
      default: return `<c r="${ref}"${sAttr} t="inlineStr"><is><t xml:space="preserve">${esc(sc.value)}</t></is></c>`;
    }
  }

  function collectEdges(props) {
    ...
    const edge = (k) => props[k] !== undefined ? parseBorderEdge(props[k], themeColors) : single;
    const edges = {
  ...
  function cfRuleXml(rule, priority, sqref) {
    ...
    if (rule.scale !== undefined) {
      const colors = rule.scale.map((c) => `<color rgb="${resolveColor(c, themeColors)}"/>`).join('');
  ...
  function validationXml(v) {
    const meta = v.meta;
    const TYPE = { list: 'list', whole: 'whole', decimal: 'decimal', date: 'date', time: 'time', 'text-length': 'textLength', custom: 'custom' };
    const OPS = { between: 'between', 'not-between': 'notBetween', '=': 'equal', '<>': 'notEqual', '>': 'greaterThan', '>=': 'greaterThanOrEqual', '<': 'lessThan', '<=': 'lessThanOrEqual' };
...
function tableStyleName(style) {
  const m = /^(light|medium|dark)-(\d+)$/.exec(style);
  if (!m) return null;
